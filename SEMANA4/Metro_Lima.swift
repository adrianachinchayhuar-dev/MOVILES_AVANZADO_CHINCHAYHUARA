...
func mostrarConexiones() {
    ...
    print("\nCada conexión está declarada explícitamente, no deducida por nombres iguales.\n")
    for c in conexiones {
...
func planificarViaje() {
    ...
    for cobro in tarifa.cobros where perfil.tarjetas.contains(cobro.sistema) {
        if perfil.saldoDe(cobro.sistema) < cobro.monto {
            print("\n  Saldo insuficiente en \(cobro.sistema.nombreCorto): "
                  + "tienes \(soles(perfil.saldoDe(cobro.sistema))), necesitas \(soles(cobro.monto)).")
        }
    }
