...
// - LÍNEAS DINÁMICAS
// ============================================================
// Las líneas 1 a 6 conservan su comportamiento original.
// Las líneas nuevas se representan como .personalizada(numero).

enum LineaMetro: Hashable {
    ...
    // Permite crear L7, L8, L9, etc.
    case personalizada(Int)
        var rawValue: Int {
        switch self {
        case .l1: return 1
        case .l2: return 2
    ...
    // Las líneas nuevas se agregan automáticamente aquí.
    static var lineasPersonalizadas: Set<Int> = []
    
    static var allCases: [LineaMetro] {
        
        let fijas: [LineaMetro] = [
            .l1, .l2, .l3,
            .l4, .l5, .l6
        ]
        
        let nuevas = lineasPersonalizadas
            .sorted()
            .map { LineaMetro.personalizada($0) }
        
        return fijas + nuevas
    }
    
    var nombre: String {
        return "Línea \(rawValue)"
    }
    ...
    var recorrido: String {
        ...
        switch self {
        case .l1:
            return "Villa El Salvador <-> Bayóvar (San Juan de Lurigancho)"
            
        case .l2:
            return "Puerto del Callao <-> Municipalidad de Ate"
            
        case .l3:
            return "El Álamo (Comas) <-> Pedro Miotta (Surco)"
            
        case .l4:
            return "Gambetta / Venezuela (Callao) <-> Mercado Santa Anita"
            
        case .l5:
            return "Parque Reducto (Miraflores) <-> Villa (Chorrillos / VES)"
            
        case .l6:
            return "Los Olivos <-> Surco"
            
        case .personalizada:
            return "Recorrido configurado por el administrador"
        }
    }
    ...
    var sistemaDePago: SistemaDePago {
        ...
        switch self {
        case .l1:
            return .tarjetaLinea1
            
        default:
            return .tarjetaInteroperable
        }
    }
    ...
    var corredor: Corredor? {
        ...
        case .l5:
            return Corredor(
                ...
                empalmeConocido:
                    "Nacería en la estación Parque Reducto de la Línea 3",
                situacion:
                    "Prevista en la red básica."
            )
        ...
        case .l6:
            return Corredor(
                ...
                empalmeConocido: nil,
                situacion:
                    "Presentada como iniciativa privada."
            )
...
nonisolated(unsafe) var redDelMetro: [LineaMetro: [Ramal]] = [
...
func crearNuevaLinea() {
    ...
    separador("Crear nueva línea")
    
    // Buscamos el siguiente número disponible.
    let numerosExistentes = LineaMetro.allCases.map { $0.rawValue }
    
    let siguienteNumero = (numerosExistentes.max() ?? 6) + 1
    
    print("\nSe creará la Línea \(siguienteNumero).")
    
    guard let nombreRamal = pedirTexto(
        "Nombre del recorrido o ramal:"
    ) else {
        return
    }
    
    guard let nombrePrimeraEstacion = pedirTexto(
        "Nombre de la primera estación:"
    ) else {
        return
    }
    
    guard let distritoPrimeraEstacion = pedirTexto(
        "Distrito de la primera estación:"
    ) else {
        return
    }
    
    let nuevaLinea = LineaMetro.personalizada(siguienteNumero)
    
    // Registramos la nueva línea.
    LineaMetro.lineasPersonalizadas.insert(siguienteNumero)
    
    // Creamos la primera estación.
    let primeraEstacion = Estacion(
        nombre: nombrePrimeraEstacion,
        distrito: distritoPrimeraEstacion,
        estado: .trazadoPublicado
    )
    
    let nuevoRamal = Ramal(
        nombre: nombreRamal,
        estaciones: [primeraEstacion]
    )
    
    // Finalmente agregamos la línea a la red.
    redDelMetro[nuevaLinea] = [nuevoRamal]
    
    print("""
    
    ✓ Línea \(siguienteNumero) creada correctamente.
    
    Línea: \(nuevaLinea.nombre)
    Ramal: \(nombreRamal)
    Primera estación: \(nombrePrimeraEstacion)
    Distrito: \(distritoPrimeraEstacion)
...
func agregarEstacionAlinea() {
    ...
    separador("Agregar estación")
    
    guard let linea = pedirLinea() else {
        return
    }
    
    guard var ramales = redDelMetro[linea],
          !ramales.isEmpty else {
        
        print("\nLa \(linea.nombre) todavía no tiene ramales registrados.")
        pausa()
        return
    }
    
    // Por simplicidad trabajamos con el primer ramal.
    // Si posteriormente se requiere administrar múltiples
    // ramales, esta función puede ampliarse.
    let indiceRamal = 0
    
    guard let nombreEstacion = pedirTexto(
        "Nombre de la nueva estación:"
    ) else {
        return
    }
    
    guard let distrito = pedirTexto(
        "Distrito:"
    ) else {
        return
    }
    
    guard let posicionTexto = pedirTexto(
        "Posición de la estación (1 = inicio):"
    ),
    let posicion = Int(posicionTexto) else {
    ...
    var estaciones = ramales[indiceRamal].estaciones
    // Validamos que no exista una estación con el mismo nombre
    // dentro de la misma línea.
    let existe = estaciones.contains {
        normalizar($0.nombre) == normalizar(nombreEstacion)
    }
    
    if existe {
        print("\n⚠ Esa estación ya existe en esta línea.")
        pausa()
        return
    }
    ...
    // Convertimos la posición humana (1, 2, 3...)
    // a índice de Swift.
    let indice = max(
        0,
        min(posicion - 1, estaciones.count)
    )
    
    estaciones.insert(nuevaEstacion, at: indice)
    
    // Ramal usa let en sus propiedades, por eso creamos
    // nuevamente el ramal con la lista actualizada.
    ramales[indiceRamal] = Ramal(
        nombre: ramales[indiceRamal].nombre,
        estaciones: estaciones
    )
    
    // Guardamos nuevamente los ramales modificados.
    redDelMetro[linea] = ramales
...
func mostrarEstaciones(de linea: LineaMetro) {
    ...
    case .l5, .l6:
        break
    case .personalizada(let n): // <--- AGREGAR ESTA LÍNEA
            print("\n Línea \(n) personalizada creada por el usuario.")
    }
}
...
func pedirLinea() -> LineaMetro? {
    ...
    guard let texto = pedirTexto(
        "Número de línea (1-\(LineaMetro.allCases.map { $0.rawValue }.max() ?? 6)):"
    ) else {
        return nil
    }
    
    guard let n = Int(texto),
          let linea = LineaMetro(rawValue: n) else {
...
func ejecutar() {
    ...
    while seguir {
        ...
        default:
            print("Opción no válida. Elige un número del 1 al 12.")
        }
    }
}

// Conectar funciones con el Administrador
...
func modoAdministrador() {
    ...
    separador("Modo administrador")
    
    print("""
    
    El modo administrador permite gestionar
    el crecimiento de la red del Metro.
    
    Funciones disponibles:
    • Ver líneas registradas
    • Agregar estaciones
    • Crear nuevas líneas
    • Ver crecimiento de la red
    
    """)
    
    while true {
        
        guard let opcion = pedirOpcion(
            "¿Qué deseas hacer?",
            opciones: [
                "Ver líneas registradas",
                "Agregar estación a una línea",
        ...
        case 1:
            agregarEstacionAdministrador()
            
        case 2:
            crearLineaAdministrador()
