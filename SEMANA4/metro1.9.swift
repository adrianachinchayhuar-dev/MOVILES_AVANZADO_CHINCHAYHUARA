...
func pedirTipoPasajero() -> TipoPasajero {
    while true {
        print("""
...
func listarLineas() {
    ...
    for linea in LineaMetro.allCases {
        print("\n[\(linea.rawValue)] \(linea.nombre.uppercased())")
        print("    Color     : \(linea.color)")
...
func lineasQuePasanPor(estacion nombreBuscado: String) -> [LineaMetro] {
    let nombreNormalizado = normalizarTexto(nombreBuscado)
    return LineaMetro.allCases.filter { linea in
        (redDelMetro[linea] ?? []).contains { normalizarTexto($0.nombre) == nombreNormalizado }
    }
}
...
func buscarEstacion(nombre nombreBuscado: String) {
    ...
    for linea in LineaMetro.allCases {
        for estacion in redDelMetro[linea] ?? [] where normalizarTexto(estacion.nombre) == nombreNormalizado {
            encontrada = true
...
func indiceDeEstacion(_ nombre: String, en linea: LineaMetro) -> Int? {
    ...
    return (redDelMetro[linea] ?? []).firstIndex { normalizarTexto($0.nombre) == nombreNormalizado }
}
...
func calcularTarifaDeViaje(
    lineaOrigen: LineaMetro,
    lineaDestino: LineaMetro,
    tipoPasajero: TipoPasajero
) -> (costo: Double, detalle: String) {
    
    let tarifaOrigen = tipoPasajero == .adulto
        ? lineaOrigen.tarifaAdulto
        : lineaOrigen.tarifaMedioPasaje

    let tarifaDestino = tipoPasajero == .adulto
        ? lineaDestino.tarifaAdulto
        : lineaDestino.tarifaMedioPasaje
    
    if lineaOrigen == lineaDestino {
        let etiqueta = lineaOrigen.tarifaEsOficial ? "" : " (tarifa estimada)"
        return (tarifaOrigen,
                "Viaje directo en \(lineaOrigen.nombre): S/ \(formatoSoles(tarifaOrigen))\(etiqueta), pago único.")
    }
 
    if lineaOrigen.sistemaDePago == lineaDestino.sistemaDePago {
        let costo = tarifaDestino
        return (costo,
                "Ambas líneas usan el mismo medio de pago (\(lineaDestino.sistemaDePago)): se considera pago único de S/ \(formatoSoles(costo)).")
    } else {
        let costo = tarifaOrigen + tarifaDestino
        return (costo,
                ...
                "se paga cada tramo por separado -> S/ \(formatoSoles(tarifaOrigen)) + S/ \(formatoSoles(tarifaDestino)) = S/ \(formatoSoles(costo)).")
    }
}
...
func planificarViaje(origen nombreOrigen: String, destino textoBuscado: String, tipoPasajero: TipoPasajero, horaSalida: String) {
    let destinoNormalizado = normalizarTexto(textoBuscado)
    if normalizarTexto(nombreOrigen) == destinoNormalizado {
    ...
    for linea in LineaMetro.allCases {
        for estacion in redDelMetro[linea] ?? [] {
            let coincide = normalizarTexto(estacion.nombre) == destinoNormalizado
                || estacion.puntosDeInteres.contains { normalizarTexto($0).contains(destinoNormalizado) }
            if coincide {
    ...
    if !horaEstaEnServicio(horaSalida, para: lineaDelDestino) {
        print("\nLa \(lineaDelDestino.nombre) no se encuentra en servicio a las \(horaSalida).")
        return
    }
    ...
    if lineasOrigen.contains(lineaDelDestino) {
        ...
        let (costo, detalleCosto) = calcularTarifaDeViaje(lineaOrigen: lineaDelDestino, lineaDestino: lineaDelDestino, tipoPasajero: tipoPasajero)
        print("\n[Tarifa] \(detalleCosto)")
        _ = costo
    ...
    for lineaOrigen in lineasOrigen {
        ...
        for estacionO in redDelMetro[lineaOrigen] ?? [] {
            if estacionesDestino.contains(where: { $0.nombre == estacionO.nombre }) {
                ...
                let (_, detalleCosto) = calcularTarifaDeViaje(lineaOrigen: lineaOrigen, lineaDestino: lineaDelDestino, tipoPasajero: tipoPasajero)
                print("\n[Tarifa] \(detalleCosto)")
...
func convertirHoraAMinutos(_ texto: String) -> Int? {
    let partes = texto.split(separator: ":")

    guard partes.count == 2,
          let horas = Int(partes[0]),
          let minutos = Int(partes[1]),
          horas >= 0,
          horas <= 23,
          minutos >= 0,
          minutos <= 59 else {
...
func horaEstaEnServicio(_ hora: String, para linea: LineaMetro) -> Bool {
    guard let minutos = convertirHoraAMinutos(hora),
          let inicio = linea.horarioInicio,
          let fin = linea.horarioFin else {
        return false
    }
...
func pedirTextoNoVacio(_ mensaje: String) -> String {
    while true {
        print(mensaje)

        let texto = readLine()?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""

        if !texto.isEmpty {
            return texto
        }

        print("El dato no puede estar vacío. Intenta nuevamente.")
    }
}

func pedirHora() -> String {
    while true {
        print("Ingresa la hora de salida (HH:MM):")
        let hora = readLine() ?? ""

        if convertirHoraAMinutos(hora) != nil {
            return hora
        }

        print("Hora no válida. Usa el formato HH:MM, por ejemplo 08:30.")
    }
}
...
while continuarEjecucion {
    ...
    case "4":
            mostrarIntercambios()
    case "5":
            let origen = pedirTextoNoVacio("Ingresa tu estación de origen:")

            let destino = pedirTextoNoVacio(
                "Ingresa tu destino (nombre de estación o punto de interés, ej: 'Estadio Nacional'):"
            )
            let tipoPasajero = pedirTipoPasajero()
            
            let horaSalida = pedirHora()
        
            planificarViaje(origen: origen, destino: destino, tipoPasajero: tipoPasajero, horaSalida: horaSalida)
    case "6":
            mostrarTarifas()
    case "7":
